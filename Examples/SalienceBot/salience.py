...
class Salience:
    ...
    def run(self, context=None, feedback=None):
        ...
        summary = self.summarize_task(data['current_task']['document'])
        execution_results = self.execute_task(summary, context, feedback, data)
    ...
    def load_data_from_storage(self):
        ...
        ordered_list = self.fetch_ordered_task_list()
        current_task = self.determine_current_task()
        ordered_results = self.prepare_ordered_results(result, ordered_list, current_task)
    ...
    def load_results(self):
        ...
        return result_collection['documents'][0] if result_collection['documents'] else "No results found"
    ...
    @staticmethod
    def prepare_ordered_results(result, ordered_list, current_task):
        return {
            'result': result,
