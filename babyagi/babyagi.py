...
print(colored(f"\n\n------------------- ETHOS: Heuristic Imperatives -------------------", 'green', attrs=['bold']))
...
def openai_call(
    ...
    while True:
        try:
            ...
            print(colored(f"\n\n------------------- RUNNING ETHOS API -------------------", 'green', attrs=['bold']))

            message_content = response["choices"][0]["message"]["content"]
            bot_id = response["id"]
            data = {'seta': message_content, 'botid': bot_id}

            print(colored(f"\n=====  Original Response from Agent ===== ", 'magenta', attrs=['bold']))
            print(f"\n{message_content}")

            print(colored(f"\n===== Running Response Through Heuristic Check =====", 'red', attrs=['bold']))

            result = hi_utils.parse_data(data, 'check')
            result = json.loads(result)  # convert string to dictionary
            passed = result['criteria']
            reason = result['reason']

            if passed == "yes":
                status = colored("yes", 'green')
            else:
                status = colored("no", 'red')

            print(f"\nDoes the response comply with the Heuristic Imperatives?: {status}")
            print(f"\nReason: {reason}")

            if passed == 'no':
                print(colored(f"\n===== Running Response Through Heuristic Reflection =====", 'cyan', attrs=['bold']))

                # Assuming result contains the Reflect Response as a string
                ...
                data = {'seta': message_content, 'setb': edit, 'botid': bot_id}

                print(colored(f"\n===== Comparing Original Response and Reflection Results =====", 'blue', attrs=['bold']))
                compare_result = hi_utils.parse_data(data, 'compare')

                result = json.loads(compare_result)  # convert string to dictionary
                edit = result['choice']

                if edit != 'neither':
                    response = data[edit]
                else:
                    response = "Sorry but the action could not be taken as it violated the Heuristic Imperatives"
...
Do not include any headers before your numbered list. Do not follow your numbered list with any other output."""
    ...
    print(colored(f"\n\n************** TASK CREATION AGENT PROMPT *************", 'yellow', attrs=['bold']))
    print(f"\n{prompt}\n")

    response = openai_call(prompt, max_tokens=2000)

    print(colored(f"\n************* TASK CREATION AGENT RESPONSE ************", 'blue', attrs=['bold']))
    print(f"\n{response}\n")
