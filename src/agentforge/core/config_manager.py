...
class ConfigManager:
    """
    Responsible for all configuration validation, normalization, merging, and structured config object construction for
    AgentForge.
    ...
    def build_agent_config(self, raw_agent_data: Dict[str, Any]) -> AgentConfig:
        ...
        try:
            ...
            self.debug_print(
                f"Agent config for '{raw_agent_data.get('name', '<unknown>')}' built successfully.",
                raw_agent_data.get("settings", {}),
            )
            return agent_config
    ...
    def _validate_agent_config(self, raw_agent_data: Dict[str, Any]) -> None:
        ...
        for key in required_keys:
            if key not in raw_agent_data:
                raise ValueError(
                    f"Agent config missing required key '{key}' for agent '{raw_agent_data.get('name', '<unknown>')}'."
                )
        ...
        if not prompts or not isinstance(prompts, dict):
            raise ValueError(
                f"Agent '{raw_agent_data.get('name', '<unknown>')}' must have a non-empty 'prompts' dictionary."
            )
        ...
        if persona is not None and not isinstance(persona, dict):
            ...
            self.debug_print(
                f"Agent '{raw_agent_data.get('name', '<unknown>')}' persona should be a dictionary, got {persona_type}",
                raw_agent_data.get("settings", {}),
            )
    ...
    def _validate_prompt_format(self, prompts: Dict[str, Any]) -> None:
        ...
        if unsupported:
            raise ValueError(
                "Prompts should contain only 'system' and 'user' keys. Please check the prompt YAML file format."
            )
    ...
    def _normalize_cog_config(self, raw_cog_data: Dict[str, Any]) -> CogConfig:
        ...
        return CogConfig(cog=cog_def, custom_fields=custom_fields)
    ...
    def _build_settings(self, raw_settings: Dict[str, Any]) -> Settings:
        ...
        debug_settings = DebugSettings(
            ...
            simulated_response=raw_system.get("debug", {}).get(
                "simulated_response",
        ...
        misc_settings = MiscSettings(on_the_fly=raw_system.get("misc", {}).get("on_the_fly", True))

        path_settings = PathSettings(files=raw_system.get("paths", {}).get("files", "./files"))
        ...
        return Settings(
            system=system_settings, models=raw_settings.get("models", {}), storage=raw_settings.get("storage", {})
        )
    ...
    def _build_cog_flow(self, raw_flow: Dict[str, Any] | None) -> CogFlow | None:
        ...
        return CogFlow(start=raw_flow["start"], transitions=transitions)
    ...
    def _parse_flow_transition(self, transition_def: Any) -> CogFlowTransition:
        ...
        if isinstance(transition_def, str):
            ...
            return CogFlowTransition(type="direct", next_agent=transition_def)

        if isinstance(transition_def, dict):
            # Check if this is an end transition
            if "end" in transition_def:
                return CogFlowTransition(type="end", end=transition_def["end"])
    ...
    def _validate_flow_references(self, flow: CogFlow, agents: List[CogAgentDef]) -> None:
        ...
        for agent_id, transition in flow.transitions.items():
            ...
            if transition.type == "direct" and transition.next_agent:
                if transition.next_agent not in agent_ids:
                    raise ValueError(
                        f"Transition from '{agent_id}' references unknown agent '{transition.next_agent}'."
                    )
                ...
                for decision_value, target_agent in transition.decision_map.items():
                    if target_agent not in agent_ids:
                        raise ValueError(
                            f"Decision transition from '{agent_id}' references unknown agent '{target_agent}'."
                        )
        ...
        has_end_transition = any(transition.type == "end" or transition.end for transition in flow.transitions.values())
