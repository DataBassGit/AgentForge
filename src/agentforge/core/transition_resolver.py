...
class TransitionResolver:
    ...
    def get_next_agent(self, current_agent_id: str, agent_outputs: Dict[str, Any]) -> str | None:
        """
    ...
    def _handle_transition(
        self, current_agent_id: str, transition: CogFlowTransition, agent_outputs: Dict[str, Any]
    ) -> str | None:
    ...
    def _handle_direct_transition(self, transition: CogFlowTransition) -> str | None:
    ...
    def _handle_decision_transition(
        self, current_agent_id: str, transition: CogFlowTransition, agent_outputs: Dict[str, Any]
    ) -> str | None:
        """
        ...
        if transition.max_visits:
            ...
            if self._has_exceeded_max_visits(current_agent_id, transition.max_visits):
                message = (
                    f"Max visits ({transition.max_visits}) exceeded for {current_agent_id}, "
                    f"using fallback: {transition.fallback}"
                )
                self.logger.log(message, "debug", "Transition")
    ...
    def _map_decision_to_next_agent(
        self, decision_value: Any, decision_map: Dict[Any, str], fallback_branch: str | None
    ) -> str | None:
        """
    ...
    def _handle_decision(
        ...
        self.logger.log(
            f"Decision summary: key='{decision_key}', value='{decision_value}', result='{next_agent}'",
            "debug",
            "Decision",
        )
        if next_agent is None:
            self.logger.log(
                f"No matching branch found for decision value '{decision_value}' and no fallback branch defined",
                "warning",
                "Decision",
            )
            self.logger.log("No match and no fallback", "debug", "Decision")
