...
class PersonaMemory(Memory):
    ...
    def __init__(self, cog_name: str, persona: str | None = None, collection_id: str | None = None) -> None:
        """
    ...
    # -----------------------------------------------------------------
    def _load_context(
        self, keys: list[str] | None, _ctx: dict, _state: dict, num_results: int
    ) -> tuple[str, list[str]]:
    ...
    def _extract_query_keys(self, keys: list[str] | None, _ctx: dict, _state: dict) -> list[str]:
        """
    ...
    def _retrieve_initial_facts(self, query_keys: list[str], num_results: int) -> list[str]:
        ...
        results = self.storage.query_storage(
            collection_name=self.collection_name, query=query_keys, num_results=num_results
        )
    ...
    def _retrieve_semantic_facts(
        self, _ctx: dict, _state: dict, static_persona: str, initial_facts: list[str], num_results: int
    ) -> list[str]:
    ...
    def _generate_semantic_queries(
        self, _ctx: dict, _state: dict, static_persona: str, retrieved_facts: list[str], num_results: int
    ) -> list[str]:
    ...
    def _perform_semantic_search(self, queries: list[str], num_results: int) -> list[str]:
        ...
        if not queries:
            return []
        results = self.storage.query_storage(
            collection_name=self.collection_name, query=queries, num_results=num_results
        )
        return results.get("documents", [])
    ...
    def _generate_narrative(self, _ctx: dict, _state: dict, static_persona: str, facts: list[str]) -> str:
        narrative_response = self.narrative_agent.run(
            _ctx=_ctx, _state=_state, persona_static=static_persona, retrieved_facts=facts
        )
    ...
    def _get_static_persona_markdown(self) -> str:
        ...
        if hasattr(self, "cog_name") and self.cog_name:
            try:
                ...
                cog_config = {"persona": cog_data.cog.persona} if cog_data.cog.persona else None
            except Exception:
                pass
    ...
    # Memory Updating
    # -----------------------------------------------------------------
    def _determine_update_action(
        self, _ctx: dict, _state: dict, static_persona: str, facts: list[str]
    ...
    def _apply_update_action(self, action: str, new_facts: list[dict[str, Any]]) -> None:
        for fact_data in new_facts:
            ...
            if action == "add":
                ...
                fact_metadata_list = [{"type": "persona_fact", "source": "update_agent", "superseded": False}]
                self.storage.save_to_storage(
                    collection_name=self.collection_name, data=[new_fact], metadata=fact_metadata_list
                )
            elif action == "update":
    ...
    def _is_duplicate_fact(self, new_fact: str) -> bool:
        ...
        """
        try:
            results = self.storage.query_storage(collection_name=self.collection_name, query=new_fact, num_results=10)
