...
class PersonaMemory(Memory):
    ...
    def query_memory(self, query_text: Union[str, list[str]], num_results: int = 5) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            queries = self._parse_retrieval_response(retrieval_response)
            if not queries:
            ...
            narrative = self._parse_narrative_response(narrative_response)
            if not narrative:
    ...
    def update_memory(self, data: dict, context: Optional[dict] = None,
        ...
        try:
            ...
            update_action = self._parse_update_response(update_response)
            if not update_action or update_action.get('action') == 'none':
            ...
            new_fact = update_action.get('new_fact')
            
            if not new_fact:
                self.logger.warning("Update action specified but no new fact provided")
                return
            ...
            if action == 'add':
                ...
                # Mark superseded facts and add new fact
                supersedes = update_action.get('supersedes', [])
                
                # Add the new fact first
    ...
    def _parse_retrieval_response(self, response: str) -> List[str]:
        """Parse the retrieval agent's JSON response to extract queries."""
        try:
            # Try to parse using the ParsingProcessor with JSON format and default code fences
            data = self.parser.parse_by_format(response, 'json')
            if data and isinstance(data, dict):
                return data.get('queries', [])
        except (ParsingError, Exception) as e:
            self.logger.warning(f"Failed to parse retrieval response: {e}")
    ...
    def _parse_narrative_response(self, response: str) -> Optional[str]:
        """Parse the narrative agent's JSON response to extract the narrative."""
        try:
            # Try to parse using the ParsingProcessor with JSON format and default code fences
            data = self.parser.parse_by_format(response, 'json')
            if data and isinstance(data, dict):
    ...
    def _parse_update_response(self, response: str) -> Optional[Dict[str, Any]]:
        """Parse the update agent's JSON response to extract action details."""
        try:
            # Try to parse using the ParsingProcessor with JSON format and default code fences
            data = self.parser.parse_by_format(response, 'json')
            if data and isinstance(data, dict):
