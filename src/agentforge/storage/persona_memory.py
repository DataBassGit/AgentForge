...
class PersonaMemory(Memory):
    ...
    # -----------------------------------------------------------------
    def load_context(self, keys: Optional[List[str]], _ctx: dict, _state: dict) -> Tuple[str, List[str]]:
        static_persona = self.get_static_persona_markdown()
        query_keys = self.extract_query_keys(keys, _ctx, _state)
        initial_facts = self.retrieve_initial_facts(query_keys)
        return static_persona, initial_facts

    def extract_query_keys(self, keys: Optional[List[str]], _ctx: dict, _state: dict) -> List[str]:
        """
    ...
    def retrieve_initial_facts(self, query_keys: List[str]) -> List[str]:
        """
    ...
    # Semantic Retrieval
    # -----------------------------------------------------------------
    def retrieve_semantic_facts(self, _ctx: dict, _state: dict, static_persona: str, initial_facts: List[str]) -> List[str]:
        queries = self.generate_semantic_queries(_ctx, _state, static_persona, initial_facts)
        semantic_facts = self.perform_semantic_search(queries)
        return self.deduplicate_facts(initial_facts + semantic_facts)

    def generate_semantic_queries(self, _ctx: dict, _state: dict, static_persona: str, retrieved_facts: List[str]) -> List[str]:
    ...
    def perform_semantic_search(self, queries: List[str]) -> List[str]:
        """
    ...
    # -----------------------------------------------------------------
    def generate_narrative(self, _ctx: dict, _state: dict, static_persona: str, facts: List[str]) -> str:
        ...
        return self.generate_static_only_narrative(static_persona)

    def generate_static_only_narrative(self, static_persona: str) -> str:
        """
    ...
    def get_static_persona_markdown(self) -> str:
        """
    ...
    # Memory Updating
    # -----------------------------------------------------------------
    def determine_update_action(self, _ctx: dict, _state: dict, static_persona: str, facts: List[str]) -> Tuple[str, List[dict]]:
    ...
    def apply_update_action(self, action: str, new_facts: List[dict]) -> None:
        for fact_data in new_facts:
            ...
            if not new_fact:
                ...
                continue
            if action == 'add':
                if self.is_duplicate_fact(new_fact):
    ...
    def query_memory(self, query_keys: Optional[List[str]], _ctx: dict, _state: dict, num_results: int = 5) -> None:
        ...
        """
        try:
            static_persona, initial_facts = self.load_context(query_keys, _ctx, _state)
            semantic_facts = self.retrieve_semantic_facts(_ctx, _state, static_persona, initial_facts)
            narrative = self.generate_narrative(_ctx, _state, static_persona, semantic_facts)
            ...
            static_persona = self.get_static_persona_markdown()
            narrative = self.generate_static_only_narrative(static_persona)
            self.narrative = narrative
            self.store = {
                '_narrative': narrative,
                '_static': static_persona,
    ...
    def update_memory(self, update_keys: Optional[List[str]], _ctx: dict, _state: dict,
        ...
        """
        try:
            static_persona, initial_facts = self.load_context(update_keys, _ctx, _state)
            semantic_facts = self.retrieve_semantic_facts(_ctx, _state, static_persona, initial_facts)
            action, new_facts = self.determine_update_action(_ctx, _state, static_persona, semantic_facts)
            if action != 'none':
                self.apply_update_action(action, new_facts)
        except Exception as e:
            self.logger.error(f"Error in update_memory: {e}")

    # -----------------------------------------------------------------
    # Storage Helpers (if any needed in future)
    # -----------------------------------------------------------------
    ...
    # -----------------------------------------------------------------
    # Persona Context & Retrieval Helpers
    # -----------------------------------------------------------------
    def _build_initial_context(self, keys: Optional[List[str]], _ctx: dict, _state: dict) -> Tuple[str, List[str]]:
        """
        Prepare persona context and perform initial fact retrieval.
        Args:
            keys: Keys to extract from context/state.
            _ctx: External context data.
            _state: Internal state data.
        Returns:
            Tuple of (static_persona, retrieved_facts).
        """
        static_persona = self._get_static_persona_markdown()
        query_keys = self._extract_query_keys(keys, _ctx, _state)
        retrieved_facts = self._initial_fact_lookup(query_keys)
        return static_persona, retrieved_facts
    ...
    def _initial_fact_lookup(self, query_keys: List[str]) -> List[str]:
        """
        Perform initial storage lookup for persona facts.
        Args:
            query_keys: List of query keys.
    ...
    def _retrieve_semantic_facts(self, _ctx: dict, _state: dict, static_persona: str, retrieved_facts: List[str]) -> List[str]:
        """
        Use retrieval agent and semantic search to get additional persona facts.
        Args:
            _ctx: External context data.
            _state: Internal state data.
            static_persona: Static persona markdown.
            retrieved_facts: Initial retrieved facts.
        Returns:
            List of unique persona facts.
        """
        queries = self._generate_retrieval_queries(_ctx, _state, static_persona, retrieved_facts)
        all_facts = list(retrieved_facts)
        if queries:
            semantic_facts = self._perform_semantic_search(queries)
            all_facts.extend(semantic_facts)
        unique_facts = self._deduplicate_facts(all_facts)
        return unique_facts

    def _generate_retrieval_queries(self, _ctx: dict, _state: dict, static_persona: str, retrieved_facts: List[str]) -> List[str]:
        """
        Call the retrieval agent to generate semantic search queries.
        Args:
            _ctx: External context data.
            _state: Internal state data.
            static_persona: Static persona markdown.
    ...
    def _perform_semantic_search(self, queries: List[str]) -> List[str]:
        """
        Perform semantic search using generated queries.
        Args:
            queries: List of queries.
    ...
    def _generate_static_only_narrative(self, static_persona: str) -> None:
        """
        Generate a narrative using only static persona when no dynamic facts are available.
        Updates self.store with the static narrative instead of returning data.
        ...
        self.logger.debug("Generated static-only narrative due to lack of dynamic facts") 