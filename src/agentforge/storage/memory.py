...
class Memory:
    ...
    def __init__(
        ...
        self.logger.debug(
            f"Initialized Memory for cog='{self.cog_name}', "
            f"persona='{self.persona}', collection='{self.collection_name}'"
        )
    ...
    def _initialize_attributes(
        self, cog_name: str, persona: str | None, collection_id: str | None, logger_name: str | None
    ) -> None:
    ...
    # Query Methods
    # -----------------------------------------------------------------
    def query_memory(self, query_keys: list[str] | None, _ctx: dict, _state: dict, num_results: int = 5) -> None:
        """
    ...
    def _execute_query(self, queries, num_results):
        ...
        """
        return self.storage.query_storage(collection_name=self.collection_name, query=queries, num_results=num_results)
    ...
    def _save_update(self, processed_data, ids, metadata_list):
        ...
        self.storage.save_to_storage(
            collection_name=self.collection_name, data=processed_data, ids=ids, metadata=metadata_list
        )
    ...
    def _build_collection_name(self) -> str:
        ...
        """
        return self.collection_id if self.collection_id else f"{self.cog_name}_{self.persona}"

    # Future: review method and consider making it optional, defaulting to PromptProcessor for automatic formatting.
    ...
    @staticmethod
    def format_memory_results(raw_results: dict) -> list[dict[str, Any]]:
        """
        ...
        Returns:
            list[dict[str, Any]]: The current readable memory result shape.
        """
        ids = raw_results.get("ids", [])
        documents = raw_results.get("documents", [])
        metadatas = raw_results.get("metadatas", [])

        records = [{"id": id_, "content": d, "meta": m} for id_, d, m in list(zip(ids, documents, metadatas))]
        ...
        for rec in records:
            ...
            current_record[rec_id] = [f"{rec['content']}\n", rec["meta"]]
    ...
    def _extract_keys(self, key_list: list[str] | None, _ctx: dict, _state: dict) -> dict:
        """
    ...
    def _extract_data_for_update(self, update_keys: list[str] | None, _ctx: dict, _state: dict) -> dict:
        """
    ...
    def _generate_metadata(self, flattened_data: dict, _ctx: dict, _state: dict) -> dict:
        ...
        if merged_context:
            context_metadata = {f"context_{k}": v for k, v in ParsingProcessor.flatten_dict(merged_context).items()}
            metadata_dict.update(context_metadata)
        return metadata_dict

    def _prepare_update_data(
        self, update_keys: list[str] | None, _ctx: dict, _state: dict, custom_metadata: list[dict] | None = None
    ) -> tuple:
        """
