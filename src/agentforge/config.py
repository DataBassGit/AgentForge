"""
Configuration loading and normalization for AgentForge.

This module owns discovery of the active `.agentforge` project root, YAML
loading for project settings and resources, and construction of structured
Agent and Cog configuration objects through ConfigManager. It preserves the
historical singleton Config interface while keeping module-level YAML loading
available for focused callers.
"""
...
class Config:
    ...
    def _initialize_attributes(self, root_path: str | None):
        """
    ...
    def find_project_root(self, root_path: str | None = None) -> pathlib.Path:
    ...
    @staticmethod
    def _merge_params(
        api_section: Dict[str, Any], class_name: str, model_data: Dict[str, Any], agent_params_override: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    @staticmethod
    def resolve_class(full_class_path: str, default_class: type | None = None, context: str = "") -> type:
        """
        ...
        if len(parts) < 2:
            raise ValueError(
                f"Invalid type format for {context}: '{full_class_path}'. "
                "Must be fully qualified (e.g., 'mymodule.MyClass')."
            )
    ...
    def _get_module(api_name: str) -> ModuleType:
        """
        Retrieves the module for the given API.
        Tries the built-in apis folder first; if not found, loads from the custom_apis folder.
        """
    ...
    @staticmethod
    def _try_load_built_in_api(api_name: str) -> ModuleType | None:
        """
    ...
    def _load_custom_api(api_name: str) -> ModuleType:
        ...
        if not custom_api_path.exists():
            raise ImportError(f"Cannot find API module '{api_name}' in the built-in folder or at {custom_api_path}.")
    ...
    def find_config(self, category: str, config_name: str) -> Dict[str, Any]:
        """
        ...
        def search_nested_dict(nested_dict: Dict[str, Any], target: str) -> Dict[str, Any] | None:
    ...
    def find_file_in_directory(self, directory: str, filename: str) -> pathlib.Path | None:
        """
        ...
        """
        search_dir = pathlib.Path(self.config_path) / directory
    ...
    def fix_prompt_placeholders(self, prompts):
        """
        Recursively traverse prompts and convert mappings like {'user_input': None} into strings like '{user_input}'.
    ...
    def resolve_persona(
        self, cog_config: Dict[str, Any] | None = None, agent_config: Dict[str, Any] | None = None
    ) -> Dict[str, Any] | None:
        """
    ...
    def load_persona(self, agent_config: dict) -> Dict[str, Any] | None:
        """
