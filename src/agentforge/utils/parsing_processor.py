...
class ParsingProcessor:
    ...
    def _define_parsers(self):
        ...
        self.parsers: Dict[str, Callable[[str, List[str] | None], Any]] = {
    ...
    def extract_code_block(self, text: str, code_fences: List[str] | None = None) -> Tuple[str | None, str]:
        """
        ...
        for fence in code_fences:
            ...
            pattern = rf"{escaped_fence}([a-zA-Z]*)[ \t]*\r?\n?([\s\S]*?){escaped_fence}"
    ...
    def sanitize_yaml_content(
        self, content: str, primary_fence: str | None = None, alternate_fence: str | None = None
    ) -> str:
        """
        ...
        pattern = rf"{re.escape(primary_fence)}[a-zA-Z]*\s*\r?\n[\s\S]*?{re.escape(primary_fence)}"
    ...
    def parse_content(
        ...
        code_fences: List[str] | None = None,
    ) -> Any:
        """
        ...
        if language is not None:
            self.logger.debug(
                f"Code-fenced block detected with language '{language}' for {expected_language.upper()} parsing"
            )
        elif code_fences:
            extracted_length = len(extracted_content)
            self.logger.debug(
                f"No code-fenced block found using fences {code_fences}, extracted content length: {extracted_length}"
            )
        else:
            self.logger.debug(
                f"No code fences specified, proceeding with full content for {expected_language.upper()} parsing"
            )
        ...
        if extracted_content:
            try:
                ...
                if expected_language.lower() == "json":
                    ...
                    self.logger.debug(
                        f"Cleaned JSON string before parsing (first 500 chars): {processed_content[:500]}"
                    )
                processed_length = len(processed_content)
                self.logger.debug(
                    f"Attempting code-fenced {expected_language.upper()} parsing on content of length "
                    f"{processed_length}"
                )
    ...
    def _attempt_bare_parsing(
        self, content_string: str, parser_func: Callable[[str], Any], expected_language: str
    ) -> Any:
        """
        ...
        self.logger.info(
            f"Attempting bare {expected_language.upper()} parsing fallback on full content (length: {stripped_length})"
        )
    ...
    def parse_by_format(
        self, content_string: str, parser_type: str | None, code_fences: List[str] | None = None
    ) -> Any:
        """
        ...
        Args:
            content_string: The input string to parse
            parser_type: Format type to parse (json, yaml, xml, ini, csv, markdown), or None to return unchanged.
            code_fences: List of fence markers to look for. If None, uses DEFAULT_CODE_FENCES (['```']).
                        Pass an empty list [] to disable code fence extraction and parse full content.

        Returns:
            Parsed content in the appropriate Python data structure, or the original content if parser_type is None.
    ...
    def auto_parse_content(self, text: str, code_fences: List[str] | None = None) -> Any:
        """
    ...
    @staticmethod
    def parse_markdown_to_dict(markdown_text: str, min_heading_level=2, max_heading_level=6) -> Dict[str, Any] | None:
        parsed_dict = {}
    ...
    def flatten_to_string_list(data) -> list[str]:
        """
        Recursively flattens any dict or list into a list of 'key: value' strings.
