...
#
#     def extract_code_block(self, text: str) -> Optional[tuple[str, Optional[str]]]:
#         """
...
#
#         Returns:
#             Optional[Tuple[str, Optional[str]]]: A tuple containing the code block content and the language specifier (or None).
#         """
#         try:
...
#             self.logger.parsing_error(content_string, e)
#             return None
#         except Exception as e:
...
#
#     @staticmethod
#     def parse_markdown_to_dict(markdown_text, min_heading_level=2, max_heading_level=6):
#         """
#         Parses a markdown-formatted string into a dictionary, mapping each heading to its corresponding content.
#
#         The function identifies headings based on markdown syntax (e.g., `#`, `##`, `###`, etc.) within a specified range
#         of heading levels and associates all subsequent lines of text with the most recent heading until the next heading
#         is encountered. The content under each heading is stored as a single string, preserving the original formatting,
#         including lists and paragraphs.
#
#         Parameters:
#             markdown_text (str): The markdown-formatted text to parse.
#             min_heading_level (int, optional): The minimum heading level to include (e.g., 1 for `#`, 2 for `##`).
#                                                Default is 2.
#             max_heading_level (int, optional): The maximum heading level to include (up to 6 for `######`).
#                                                Default is 6.
#
#         Returns:
#             dict: A dictionary where each key is a heading (str) and each value is the associated content (str).
#
#         Notes:
#             - Headings outside the specified level range are ignored.
#             - Ensure headings are properly formatted with at least one space after the `#` symbols.
#         """
#         parsed_dict = {}
