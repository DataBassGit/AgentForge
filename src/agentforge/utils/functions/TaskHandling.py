...
class TaskHandling:
    ...
    def get_current_task(self):
        ...
        #TODO: this returns None and breaks the code, it needs some kind of catch here to return an object that can still be processed by the class calling it during the event loop.
        current_task = {}
    ...
    def get_ordered_task_list(self):
        ...
        task_collection = self.storage.storage_utils.load_collection(
            {"collection_name": "Tasks", "include": ["documents", "metadatas"]}
        )

        # first, pair up 'ids', 'documents' and 'metadatas' for sorting
        paired_up_tasks = list(
            zip(
                task_collection["ids"],
