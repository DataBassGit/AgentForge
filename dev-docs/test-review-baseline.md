...
This is a deterministic review baseline for test cleanup, not a full test-suite audit.

## Collection Baseline

- Default collection before the review cleanup: `192/193` tests selected, with one integration-marked test deselected by `pytest.ini`.
- Default collection after the review cleanup: `193/194` tests selected, with one integration-marked test deselected by `pytest.ini`.
- Integration marker before the review cleanup: only `tests/multimedia_tests/test_vision_support.py::TestGeminiVision::test_real_image_processing`.
- Integration marker after the review cleanup: unchanged; only `tests/multimedia_tests/test_vision_support.py::TestGeminiVision::test_real_image_processing`.
...
- `tests/integration_tests/test_persona_memory_integration.py` contained only fixtures before the review cleanup, so it collected no tests; it now collects one fast fake-backed PersonaMemory workflow test.
...
| Noisy/ceremonial tests | Some `ConfigManager` validation checks used try/except, `assert False`, and print-only success messages. | Convert the touched validation checks to `pytest.raises(..., match=...)`. |
...
| --- | --- | --- | --- | --- | --- |
| `tests/config_tests/test_config_manager_validation.py::test_config_manager_validation` | Invalid agent and Cog config fail with actionable `ValueError` messages. | Missing required fields accidentally accepted or diagnostics changed into opaque failures. | Config loader and validation. | Fake-backed default. | Updated to direct `pytest.raises` assertions. |
...
- `tests/config_tests/test_config_manager_validation.py` no longer emits print-only success output; assertions remain the test proof.
...
- Focused config/Cog/core tests passed with `31 passed`, and the full default suite passed with `225 passed, 1 deselected`.
...
- Continue replacing print-only success messages in tests outside the current clean scope when those files are touched for substantive reasons.
