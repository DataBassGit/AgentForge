...
class TestTransitionResolver:
    ...
    def test_end_transition(self):
        ...
        # Setup
        transitions = {"agent1": CogFlowTransition(type="end", end=True)}
        flow = CogFlow(start="agent1", transitions=transitions)
        resolver = TransitionResolver(flow)
    ...
    def test_get_transition_missing_agent_raises_exception(self):
        ...
        transitions = {"agent1": CogFlowTransition(type="direct", next_agent="agent2")}
        flow = CogFlow(start="agent1", transitions=transitions)
        resolver = TransitionResolver(flow)
    ...
    def test_decision_transition_with_match(self):
        ...
        agent_outputs = {"decide_agent": {"choice": "approve"}}
    ...
    def test_decision_transition_with_fallback(self):
        ...
        agent_outputs = {"decide_agent": {"choice": "unknown_value"}}
    ...
    def test_decision_transition_missing_key_uses_fallback(self):
        ...
        transitions = {
            "decide_agent": CogFlowTransition(
                type="decision", decision_key="choice", decision_map={"approve": "agent2"}, fallback="agent4"
            )
        }
        flow = CogFlow(start="decide_agent", transitions=transitions)
        resolver = TransitionResolver(flow)
    ...
    def test_decision_transition_case_insensitive_matching(self):
        ...
        for decision_value, expected_agent in test_cases:
            agent_outputs = {"decide_agent": {"choice": decision_value}}
    ...
    def test_max_visits_exceeded_uses_fallback(self):
        ...
        agent_outputs = {"loop_agent": {"choice": "continue"}}
    ...
    def test_no_decision_key_uses_fallback(self):
        ...
        # Setup
        transitions = {
            "agent1": CogFlowTransition(type="decision", decision_key=None, decision_map={}, fallback="agent2")
        }
        flow = CogFlow(start="agent1", transitions=transitions)
        resolver = TransitionResolver(flow)
    ...
    def test_decision_transition_with_no_fallback_returns_none(self):
        ...
        transitions = {
            "decide_agent": CogFlowTransition(
                type="decision", decision_key="choice", decision_map={"approve": "agent2"}, fallback=None
            )
        }
        flow = CogFlow(start="decide_agent", transitions=transitions)
        resolver = TransitionResolver(flow)
    ...
    def test_mixed_type_decision_values(self):
        ...
        for decision_value, expected_agent in test_cases:
            ...
            assert next_agent == expected_agent, (
                f"Failed for decision value: {decision_value} (type: {type(decision_value)})"
            )
