...
def test_disabled_chat_memory_is_not_initialized_loaded_or_recorded(isolated_config):
    ...
    with (
        patch.object(ChatHistoryMemory, "query_memory", autospec=True) as query_chat,
...
def test_memory_manager_get_dot_notated():
    ...
    data = {"direct": "value", "level1": {"simple": "test", "level2": {"value": "found"}}}
    ...
    # Test non-dict input
    non_dict_source: Any = "not a dict"
    assert ParsingProcessor.get_dot_notated(non_dict_source, "key") is None
