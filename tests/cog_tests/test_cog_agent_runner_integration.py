...
class TestCogAgentRunnerIntegration:
    ...
    def test_cog_agent_runner_preserves_retry_behavior(self, isolated_config):
        ...
        cog_config = {
            "cog": {
                "agents": [{"id": "flaky_agent", "template_file": "test_template"}],
                "flow": {"start": "flaky_agent", "transitions": {"flaky_agent": {"end": True}}},
            }
        }
    ...
    def test_cog_preserves_memory_integration_with_agent_runner(self, isolated_config):
        ...
        cog_config = {
            "cog": {
                "agents": [{"id": "memory_agent", "template_file": "test_template"}],
                "memory": [
                    {
                        "id": "test_memory",
                ...
                "flow": {"start": "memory_agent", "transitions": {"memory_agent": {"end": True}}},
            }
        }
        ...
        # Create Cog
        cog = Cog("MemoryCog")
        cog.agents["memory_agent"] = Mock(run=Mock(return_value="Memory test response"))

        # Mock memory manager methods to verify they're called
        with (
            patch.object(cog.mem_mgr, "query_before") as mock_query,
