...
class TestCogTrailLoggingAndFlowValidation:
    ...
    def test_trail_logging_from_yaml_config(self, isolated_config):
        ...
        cog_config = {
            "cog": {
                "trail_logging": False,
                "agents": [{"id": "test_agent", "template_file": "test_template"}],
                "flow": {"start": "test_agent", "transitions": {"test_agent": {"end": True}}},
            }
        }
    ...
    def test_flow_validation_warns_on_missing_end_state(self, isolated_config):
        ...
        assert flow.start == "agent1", "Start agent should be correct"
        assert len(flow.transitions) == 2, "Should have 2 transitions"
    ...
    def test_flow_validation_no_warning_with_valid_end_state(self, isolated_config):
        ...
        agent2_transition = flow.transitions["agent2"]
    ...
    def test_flow_validation_allows_cycles_with_warning(self, isolated_config):
        ...
        assert flow.start == "agent1", "Start agent should be correct"
        assert len(flow.transitions) == 3, "Should have 3 transitions"

        # Verify the cycle structure
        assert flow.transitions["agent1"].next_agent == "agent2"
        assert flow.transitions["agent2"].next_agent == "agent3"
        assert flow.transitions["agent3"].next_agent == "agent1"
