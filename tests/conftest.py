...
from tests.utils.fakes import FakeChromaStorage  # noqa: E402
...
@pytest.fixture()
def isolated_config(tmp_path) -> Generator[Config, None, None]:
...
@pytest.fixture()
def clean_yaml_after_test() -> Generator[None, None, None]:
    ...
    if original_content is not None and example_cog_path.parent.exists():
...
def stubbed_agents(monkeypatch):
    ...
    def fake_run(self: Agent, **context):  # type: ignore[override]
        ...
        if isinstance(settings, dict) and settings.get("system", {}).get("debug", {}).get("mode", False):
