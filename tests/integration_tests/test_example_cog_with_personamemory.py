...
class TestExampleCogWithPersonaMemoryIntegration:
    ...
    def real_agent_responses(self):
        ...
        responses = {
            ...
            "narrative_agent": {
                "narrative": (
                    "I am the Default Assistant, a helpful and friendly entity existing within a virtual environment. "
                    "My purpose is to assist users by providing clear, concise, and accurate information, always "
                    "striving to be knowledgeable and as helpful as possible in responding to their questions and "
                    "needs."
                )
            },
            ...
            "understand_agent": {
                "insights": (
                    "The user is initiating a casual conversation with a friendly greeting. They are likely testing "
                    "the assistant's conversational abilities and responsiveness."
                ),
            ...
            "persona_response_agent": (
                "Heyo! I'm doing well, thanks for asking. Just here and ready to help. How can I assist you today?"
            ),
            "update_agent": {"action": "none", "new_facts": []},
            "persona_update_agent": {"action": "none", "new_facts": []},
        }
    ...
    def test_complete_cog_workflow_with_persona_memory(
        self, isolated_config, fake_chroma, mock_agent_responses_with_real_data, real_agent_responses
    ):
        """
        ...
        assert mock_agent_responses_with_real_data["persona_retrieval_agent"] >= 1, (
            "Retrieval agent should be called for persona memory"
        )
        assert mock_agent_responses_with_real_data["persona_narrative_agent"] >= 1, (
            "Narrative agent should be called for persona context"
        )
        assert mock_agent_responses_with_real_data["persona_understand_agent"] == 1, (
            "Understand agent should be called once"
        )
        assert mock_agent_responses_with_real_data["persona_response_agent"] == 1, (
            "Response agent should be called once"
        )
        assert mock_agent_responses_with_real_data["persona_update_agent"] >= 1, (
            "Update agent should be called for memory updates"
        )
        ...
        assert cog.branch_call_counts.get("persona_understand_agent", 0) == 0, (
            "branch_call_counts should be reset after loop exit"
        )

    def test_persona_memory_integration_with_context_resolution(
        self, isolated_config, fake_chroma, mock_agent_responses_with_real_data
    ):
        """
    ...
    def test_memory_query_and_update_lifecycle(self, isolated_config, fake_chroma, mock_agent_responses_with_real_data):
        """
        ...
        # Execute the cog
        cog.run(user_input="I love programming in Python")
        ...
        assert mock_agent_responses_with_real_data["persona_retrieval_agent"] >= 2, (
            "Retrieval agent called for both query and update"
        )
