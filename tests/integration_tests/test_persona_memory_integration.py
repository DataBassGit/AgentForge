...
class TestPersonaMemoryIntegration:
    ...
    def cog_config_with_persona_memory(self, isolated_config, fake_chroma):
        ...
        persona_memory_cog = {
            "cog": {
                ...
                "flow": {"start": "understand", "transitions": {"understand": "respond", "respond": {"end": True}}},
            }
        }
        ...
        retrieval_agent_prompts = {
            "prompts": {
                "system": {"intro": "You are the Retrieval Agent for persona memory.", "context": "{context}"},
        ...
        update_agent_prompts = {
            "prompts": {
                "system": {"intro": "You are the Update Agent for persona memory.", "context": "{context}"},
        ...
        isolated_config.data.setdefault("prompts", {})["persona_retrieval_agent"] = retrieval_agent_prompts
        isolated_config.data.setdefault("prompts", {})["persona_narrative_agent"] = narrative_agent_prompts
        isolated_config.data.setdefault("prompts", {})["persona_update_agent"] = update_agent_prompts
        ...
        isolated_config.data.setdefault("personas", {})["default_assistant"] = {
            ...
            "retrieval": {"tone": "Professional", "expertise": ["Testing", "AI"]},
        }

        # Mock the config methods that Cog uses
        with (
            patch.object(isolated_config, "load_cog_data") as mock_load_cog,
            ...
            mock_load_cog.return_value = isolated_config.config_manager.build_cog_config(persona_memory_cog)
            ...
            def load_agent_side_effect(agent_name):
                ...
                return isolated_config.config_manager.build_agent_config(base_data)
    ...
    def mock_persona_agents(self, monkeypatch):
        ...
        def enhanced_fake_run(self: Agent, **context):
            ...
            if agent_name == "persona_retrieval_agent":
                return {"queries": ["user preferences", "interaction patterns"]}
            elif agent_name == "persona_narrative_agent":
                return {
                    "narrative": "User is a developer who prefers concise explanations and enjoys Python programming."
                }
            elif agent_name == "persona_update_agent":
    ...
    def test_persona_memory_cog_queries_and_updates_with_fakes(
        self, cog_config_with_persona_memory, fake_chroma, mock_persona_agents
    ):
        """PersonaMemory should query and update during a fake-backed Cog workflow."""
        cog = Cog("ExampleCogWithPersonaMemory")

        result = cog.run(user_input="I like concise Python examples")

        assert result == "FINAL RESPONSE"
        assert "persona_memory" in cog.mem_mgr.memory_nodes
        persona_memory = cog.mem_mgr.memory_nodes["persona_memory"]["instance"]
        assert persona_memory.store["_narrative"] == (
            "User is a developer who prefers concise explanations and enjoys Python programming."
        )
        assert mock_persona_agents["retrieval"].call_count >= 2
        assert mock_persona_agents["narrative"].call_count == 1
        assert mock_persona_agents["update"].call_count == 1
        stored = persona_memory.storage.peek(persona_memory.collection_name)["documents"]
        assert "User enjoys discussing programming topics" in stored
